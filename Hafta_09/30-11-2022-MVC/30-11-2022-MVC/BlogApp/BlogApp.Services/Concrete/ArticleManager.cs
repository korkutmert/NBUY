...
namespace BlogApp.Services.Concrete
{
    public class ArticleManager : IArticleService
    {
        ...
        public async Task<IResult> Add(ArticleAddDto articleAddDto, string createdByName)
        {
            ...
            article.UserId = 1;
            #region Açıklamalar
            //UserId yı sımdılık elle verıyoruz.Logın ıslemlerını tamamladıgımızda burayı degıstırecegız(Login işlemı yok)
            //Eger entitybase ıcerısınde createddate ıcın default deger vermemıs olsaydık boyle yapabılırdık.Bunun ıkıncı bır yolu da profile ıcınden bu degerı vermektır.
        ...
        public async Task<IResult> Update(ArticleUpdateDto articleUpdateDto, string modifiedByName)
        {
            ...
            var result = await _unitOfWork.Articles.GetAsync
                (a=>a.Id==articleUpdateDto.Id);
            if (result!=null)
            {
